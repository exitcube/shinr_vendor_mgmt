...
import { verifyVendorAccessToken, verifyAdminAccessToken, verifyVendorOrgAccessToken } from './jwt';
import { createErrorResponse } from './response';
import { vendorAccessTokenPayloadType, AuthenticatedUser, adminAccessTokenPayloadType, AdminAuthenticatedUser, VendorAuthenticatedUser, vendorOrgAccessTokenPayloadType, VendorOrgAuthenticatedUser } from '../types/config';
...
/**
 * PreHandler to validate JWT token for vendorOrg
 */
export async function vendorOrgAuthValidationPreHandler(
    ...
    try {
        ...
        // Verify the JWT token
        try {
            const payload : vendorOrgAccessTokenPayloadType= await verifyVendorOrgAccessToken(token);
            // Attach vendorOrg info to request for use in handlers
            const vendorOrgInfo: VendorOrgAuthenticatedUser = {
                vendorOrganizationId: payload.vendorOrganizationId,
                tokenId: payload.tokenId,
            };
            (request as any).user = vendorOrgInfo;
        } catch (jwtError: any) {
            let errorCode = 'INVALID_TOKEN';
            let errorMessage = 'Invalid token';
            let publicMessage = 'The provided token is invalid';
