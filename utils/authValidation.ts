...
import { verifyVendorAccessToken, verifyVendorOrgAccessToken} from './jwt';
import { createErrorResponse } from './response';
import { vendorOrgAccessTokenPayloadType, AuthenticatedUser, adminAccessTokenPayloadType, AdminAuthenticatedUser, VendorOrgAuthenticatedUser, vendorAccessTokenPayloadType, VendorAuthenticatedUser } from '../types/config';
...
/**
 * PreHandler to validate JWT token for vendor
 */
export async function vendorAuthValidationPreHandler(
    request: FastifyRequest,
    reply: FastifyReply
): Promise<void> {
    try {
        ...
        // Verify the JWT token
        try {
            const payload : vendorAccessTokenPayloadType= await verifyVendorAccessToken(token);
            // Attach vendorOrg info to request for use in handlers
            const vendorInfo: VendorAuthenticatedUser = {
                vendorId: payload.vendorId,
                tokenId: payload.tokenId,
            };
            (request as any).user = vendorInfo;
        } catch (jwtError: any) {
            let errorCode = 'INVALID_TOKEN';
            let errorMessage = 'Invalid token';
            let publicMessage = 'The provided token is invalid';
